...
export async function updateEmail(data: UpdateEmailData): Promise<{ success: boolean; message: string }> {
  ...
  if (!response.ok) throw new Error(result.message || "Erreur lors de la mise à jour de l'email");
  return result;
}
...
export async function updatePassword(data: UpdatePasswordData): Promise<{ success: boolean; message: string }> {
  ...
  const result = await response.json();
  if (!response.ok) throw new Error(result.message || 'Erreur lors de la mise à jour du mot de passe');
  return result;
}
...
export async function updateBackupEmail(data: UpdateBackupEmailData): Promise<{ success: boolean; message: string }> {
  ...
  const result = await response.json();
  if (!response.ok) throw new Error(result.message || "Erreur lors de la mise à jour de l'email de secours");
  return result;
}
